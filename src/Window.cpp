...
void DrawJointUI(Joint* joint) {
    ...
    if (ImGui::TreeNode(joint->GetName().c_str())) {
        ...
        ImGui::SliderAngle("Rotate X", &posePtr[0], glm::degrees(limX.x), glm::degrees(limX.y));
        ImGui::SliderAngle("Rotate Y", &posePtr[1], glm::degrees(limY.x), glm::degrees(limY.y));
        ImGui::SliderAngle("Rotate Z", &posePtr[2], glm::degrees(limZ.x), glm::degrees(limZ.y));
...
void Window::displayCallback(GLFWwindow* window) {
    ...
    if (skeleton) skeleton->Draw(Cam->GetViewProjectMtx(), Window::shaderProgram);
